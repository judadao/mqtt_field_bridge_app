...
".bar{display:flex;gap:10px;align-items:center;justify-content:space-between;flex-wrap:wrap}.stack{display:grid;gap:12px}.grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px}.panel,details{background:var(--panel);border:1px solid var(--line);border-radius:8px;padding:14px;margin-bottom:12px;box-shadow:0 1px 2px rgba(16,24,20,.04)}"
...
".hero{display:grid;grid-template-columns:1.2fr .8fr;gap:12px}.quick{display:grid;grid-template-columns:1fr auto;gap:12px;align-items:start}.kv{width:100%;border-collapse:collapse}.kv th,.kv td{text-align:left;border-bottom:1px solid var(--line);padding:8px 6px}.kv th{width:36%;color:var(--muted);font-size:12px;font-weight:700}.kv tr:last-child th,.kv tr:last-child td{border-bottom:0}.value{font-weight:700}details{padding:0;overflow:hidden}summary{display:flex;align-items:center;justify-content:space-between;gap:10px;padding:14px;cursor:pointer;font-weight:800;color:var(--fg)}summary:after{content:\"+\";color:var(--accent);font-size:18px}details[open] summary{border-bottom:1px solid var(--line)}details[open] summary:after{content:\"-\"}.fold-body{padding:14px}.fold-body .panel{box-shadow:none;margin-bottom:0}"
...
".toolbar{display:flex;gap:8px;flex-wrap:wrap}.hint{font-size:12px;color:var(--muted);margin-top:8px}"
...
"pre{white-space:pre-wrap;margin:0;max-height:320px;overflow:auto;background:var(--panel2);border:1px solid var(--line);border-radius:8px;padding:10px}.small{font-size:12px}@media(max-width:760px){main{padding:14px}.grid,.peer,.hero,.quick{grid-template-columns:1fr}.actions{justify-content:flex-start}}"
...
"<section id=\"app\" class=\"hide\"><section class=\"hero\"><div class=\"panel\"><h2>Overview</h2><table class=\"kv\"><tr><th>Status</th><td><span id=\"status\" class=\"pill muted\">Loading</span></td></tr><tr><th>Device IP</th><td id=\"ip-state\" class=\"value\">-</td></tr><tr><th>Gateway</th><td id=\"summary-gateway\" class=\"value\">-</td></tr><tr><th>DNS</th><td id=\"summary-dns\" class=\"value\">-</td></tr><tr><th>Broker</th><td id=\"broker-state\" class=\"value\">-</td></tr><tr><th>P2P</th><td id=\"p2p-state\" class=\"value\">-</td></tr></table></div><div class=\"panel\"><h2>Quick Actions</h2><div class=\"stack\"><div id=\"wifi-state\" class=\"pill muted\">WiFi -</div><div id=\"save-state\" class=\"pill muted\">No changes</div><div class=\"toolbar\"><button type=\"button\" id=\"broker-start\">Start Broker</button><button type=\"button\" id=\"broker-stop\" class=\"secondary\">Stop Broker</button><button type=\"button\" id=\"save-network-top\" class=\"secondary\">Save Network</button></div></div></div></section>"
"<details open><summary>Network Setting</summary><div class=\"fold-body\"><div class=\"grid\"><label>Device Default IP<input id=\"device_ip\" maxlength=\"63\"></label><label>Gateway<input id=\"gateway\" maxlength=\"63\"></label><label>Netmask<input id=\"netmask\" maxlength=\"63\"></label><label>DNS<input id=\"dns\" maxlength=\"63\"></label><label>WiFi SSID<input id=\"wifi_ssid\" maxlength=\"63\"></label><label>WiFi Password<input id=\"wifi_password\" type=\"password\" maxlength=\"63\"></label><label>DHCP Enabled<input id=\"dhcp_enabled\" type=\"checkbox\"></label><label>AP SSID<input id=\"ap_ssid\" maxlength=\"63\"></label><label>AP Password<input id=\"ap_password\" type=\"password\" maxlength=\"63\"></label></div><div class=\"actions\"><button type=\"button\" id=\"save-network\">Save Network</button></div></div></details>"
"<details><summary>Broker Setting / Bridge Mesh Setting</summary><div class=\"fold-body\"><div class=\"grid\"><label>Site ID<input id=\"site_id\" maxlength=\"31\"></label><label>Topic Prefix<input id=\"topic_prefix\" maxlength=\"63\"></label><label>MQTT Port<input id=\"cfg_mqtt_port\" type=\"number\" min=\"1\" max=\"65535\"></label><label>P2P Port<input id=\"cfg_p2p_port\" type=\"number\" min=\"1\" max=\"65535\"></label><label>Broker Enabled<input id=\"broker_enabled\" type=\"checkbox\"></label><label>Bridge Enabled<input id=\"bridge_enabled\" type=\"checkbox\"></label><label>Mesh Enabled<input id=\"mesh_enabled\" type=\"checkbox\"></label></div><div class=\"actions\"><button type=\"button\" id=\"save-broker\">Save Broker</button></div></div></details>"
"<details><summary>Bridge Peers</summary><div class=\"fold-body\"><form id=\"peer-form\"></form><div class=\"actions\"><button type=\"button\" id=\"save-all\">Save All Peers</button><button type=\"button\" id=\"disable-all\" class=\"secondary\">Disable All Peers</button></div></div></details>"
"<details><summary>System Setting</summary><div class=\"fold-body\"><div class=\"grid\"><label>Device Name<input id=\"device_name\" maxlength=\"31\"></label><label>Admin Password<input id=\"admin_password\" type=\"password\" maxlength=\"63\"></label></div><div class=\"actions\"><button type=\"button\" id=\"save-system\">Save System</button><button type=\"button\" id=\"reset-config\" class=\"secondary\">Reset Config</button></div></div></details>"
"<details><summary>Topic Test</summary><div id=\"topic-test\" class=\"fold-body\"><div class=\"grid\"><label>Topic<input id=\"test_topic\" maxlength=\"127\"></label><label>Payload<input id=\"test_payload\" maxlength=\"255\" value=\"hello\"></label><label>QoS<input id=\"test_qos\" type=\"number\" min=\"0\" max=\"1\" value=\"0\"></label><label>Retain<input id=\"test_retain\" type=\"checkbox\"></label></div><div class=\"actions\"><button type=\"button\" id=\"publish-test\">Publish Test</button><span id=\"publish-state\" class=\"muted\">No test publish</span></div></div></details>"
"<details><summary>Raw Status</summary><div class=\"fold-body\"><pre id=\"raw\" class=\"small muted\"></pre></div></details></section></main>"
"<script>"
...
"function put(c){cfg=c;for(const k in c){const e=$(k);if(!e)continue;if(e.type==='checkbox')e.checked=!!c[k];else e.value=c[k]||''}$('ip-state').textContent=c.device_ip||'-';$('summary-gateway').textContent=c.gateway||'-';$('summary-dns').textContent=c.dns||'-'}"
...
"$('refresh').onclick=load;$('save-system').onclick=()=>saveConfig('System');$('reset-config').onclick=resetConfig;$('save-network').onclick=()=>saveConfig('Network');$('save-network-top').onclick=()=>saveConfig('Network');$('save-broker').onclick=()=>saveConfig('Broker');$('broker-start').onclick=()=>brokerControl(1);$('broker-stop').onclick=()=>brokerControl(0);$('save-all').onclick=saveAll;$('disable-all').onclick=disableAll;$('publish-test').onclick=publishTest;"
"</script></body></html>";
