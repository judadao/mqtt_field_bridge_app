...
".pill{display:inline-block;border:1px solid var(--l);border-radius:99px;padding:4px 9px;color:var(--m)}.op{border:1px solid var(--l);border-radius:8px;background:var(--b);padding:10px 12px;font-weight:700;overflow-wrap:anywhere}.ok{color:#2e7d32}.err{color:var(--d)}"
...
"<section class=card><h2>Operation Result</h2><div id=o class=op>Ready</div></section>"
...
"function O(k,c){c=c||(k=='ERR'?'err':'ok');E('s').textContent=k||'OK';E('s').className='pill '+c;E('o').textContent=k||'OK';E('o').className='op '+c}"
