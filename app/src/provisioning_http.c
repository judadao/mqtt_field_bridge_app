...
"tr:last-child th,tr:last-child td{border:0}.row,.tabs{display:flex;gap:8px;flex-wrap:wrap;align-items:center}.tabs{margin:0 0 12px}.grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}"
...
".pill{display:inline-block;border:1px solid var(--l);border-radius:99px;padding:4px 9px;color:var(--m)}.op{border:1px solid var(--l);border-radius:8px;background:var(--b);padding:10px 12px;font-weight:700;overflow-wrap:anywhere}.tab{background:transparent;color:var(--a)}.tab.on{background:var(--btn);color:white}.pop{position:fixed;inset:0;z-index:9;background:rgba(15,23,42,.4);display:grid;place-items:center;padding:18px}.modal{width:min(420px,100%);border:1px solid var(--l);border-radius:8px;background:var(--p);padding:14px;box-shadow:0 18px 50px rgba(15,23,42,.28)}.hide{display:none}.ok{color:#2e7d32}.err{color:var(--d)}"
...
"<div id=pop class='pop hide'><div class=modal><h2>Operation Result</h2><div id=o class=op>Ready</div><p><button onclick=closePop()>OK</button></p></div></div>"
...
"function O(k,c,p){c=c||(k=='ERR'?'err':'ok');E('s').textContent=k||'OK';E('s').className='pill '+c;E('o').textContent=k||'OK';E('o').className='op '+c;if(p)E('pop').classList.remove('hide')}"
"function P(p,msg,pop){p.then(()=>O(msg||'OK','ok',pop)).catch(e=>O('ERR '+(e&&e.message?e.message:''),'err',1))}"
...
"function saveConfig(){O('Saving broker','muted',1);P(J('/config','POST',getCfg()).then(rebooting),'Rebooting',1)}"
"function saveNetwork(){O('Saving network','muted',1);P(J('/config','POST',getCfg()).then(rebooting),'Rebooting',1)}"
"function resetConfig(){O('Resetting','muted',1);P(J('/config/reset','POST').then(rebooting),'Rebooting',1)}"
...
"function savePeer(){let i=+E('peer_slot').value,p={name:E('peer_name').value,host:E('peer_host').value,mqtt_port:+E('peer_mqtt').value,p2p_port:+E('peer_p2p').value,enabled:E('peer_enabled').checked?1:0};O('Saving peer','muted',1);P(J('/peers/'+i,'POST',p).then(x=>J('/peers').then(p=>(peers=p.map(norm),drawSlots(),E('peer_slot').value=i,loadPeer(),x))),'Peer saved',1)}"
