...
"input[type=checkbox]{width:22px;min-height:22px;accent-color:var(--a)}button{border:1px solid var(--a);background:var(--a);color:white;border-radius:6px;padding:8px 12px;font-weight:700}button.alt{background:transparent;color:var(--a)}"
"pre{white-space:pre-wrap;word-break:break-word;background:var(--b);border:1px solid var(--l);border-radius:6px;padding:10px;min-height:44px}.pill{display:inline-block;border:1px solid var(--l);border-radius:99px;padding:4px 9px;color:var(--m)}.ok{color:#2e7d32}.err{color:var(--d)}"
...
"function O(x,k){E('o').textContent=typeof x=='string'?x:JSON.stringify(x,null,2);E('s').textContent=k||'OK';E('s').className='pill '+(k=='ERR'?'err':'ok')}"
"function P(p){p.then(v=>O(v)).catch(e=>O(e,'ERR'))}"
