...
"function O(k,c,p){c=c||(k=='ERR'?'err':'ok');E('s').textContent=k||'OK';E('s').className='pill '+c;E('o').textContent=k||'OK';E('o').className='op '+c;if(p){E('pop').classList.remove('hide');setTimeout(closePop,2000)}}"
...
"function rebooting(x){O('Rebooting','ok');return J('/reboot','POST').then(()=>x)}"
