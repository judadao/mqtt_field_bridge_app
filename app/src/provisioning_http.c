...
"<div><h2>Broker Peers</h2><p>Broker Slots</p><pre id=\"peers\"></pre>"
"<h2>WiFi Connect</h2><p>Use UART CLI for WiFi mode. Ethernet mode exposes this web UI/API.</p>"
"<h2>Operation Result</h2><pre id=\"operation-result\"></pre></div></section>"
"<script>"
"async function j(p,o){let r=await fetch(p,o);let t=await r.text();try{return JSON.parse(t)}catch(e){return t}}"
"async function load(){let c=await j('/config');for(let k of ['device_ip','gateway','netmask','dns','broker_ip','mqtt_port','p2p_port','topic_prefix']){if(document.getElementById(k))document.getElementById(k).value=c[k]||''}document.getElementById('mode').value=c.mode||'auto';document.getElementById('peers').textContent=JSON.stringify(await j('/peers'),null,2);document.getElementById('operation-result').textContent=JSON.stringify(await j('/status'),null,2)}"
"async function saveConfig(){let b={device_name:'esp32-min-broker',site_id:'field-a',dhcp_enabled:0,broker_enabled:1,bridge_enabled:1,mesh_enabled:1};for(let k of ['device_ip','gateway','netmask','dns','broker_ip','topic_prefix','mode'])b[k]=document.getElementById(k).value;for(let k of ['mqtt_port','p2p_port'])b[k]=parseInt(document.getElementById(k).value||'0',10);document.getElementById('operation-result').textContent=JSON.stringify(await j('/config',{method:'POST',body:JSON.stringify(b)}),null,2)}"
