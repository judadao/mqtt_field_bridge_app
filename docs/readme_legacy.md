...
This product now consumes these reusable repos through `deps.json`:

- `dephy`: board-platform profile and Zephyr workspace setup.
- `mqtt_min_broker`: reusable MQTT broker and optional P2P routing.
- `dephy_industrial_io`: reusable industrial IO payload/topic bridge helpers.
- `dephy_testkit`: Linux test harness and JSON result wrapper.

The product owns field configuration, provisioning HTTP/UI, peer workflow, and
product integration points. Reusable behavior should be implemented in the
module repos first, then pinned here.
...
The only open TODO items are hardware-blocked ESP32 validation tasks in
`docs/todo.yaml`: WiFi reconnect, live WiFi scan, and DHCP/IP role validation.
...
For a fast local check that avoids long stress/integration paths:
