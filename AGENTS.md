...
This repository is the product layer for an MQTT field bridge. Core firmware code lives in `app/src/`, with Zephyr configuration in `app/prj.conf` and `app/CMakeLists.txt`. The provisioning UI source is in `app/web/`; it is embedded into `app/src/generated/provisioning_index.h` by `scripts/build_provisioning_web.js`. Linux host tests and helpers live in `tests/linux/`. Project notes and scenarios are under `docs/`. The MQTT broker dependency is pinned in `deps.json` and synced into `deps/mqtt_min_broker/`; do not rely on an arbitrary local broker checkout.

## Build, Test, and Development Commands

- `./scripts/sync_deps.sh`: sync the pinned broker dependency.
- `./scripts/sync_deps.sh --check-latest`: check whether a newer broker release exists.
- `./scripts/build_product.sh`: run a Zephyr `west build` for the board from `deps.json` or `esp32`.
